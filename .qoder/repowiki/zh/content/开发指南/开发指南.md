...
- 构建与运行
  - Ruby 与 Jekyll 依赖在 Gemfile 中声明。
  - 本地开发通过 run_server.sh 启动带热重载的服务。
...
- 本地开发
  - 通过 run_server.sh 启动 Jekyll 服务并启用 livereload，修改源码后自动刷新。
- 部署
  - 将仓库推送到 GitHub Pages 即可自动构建与发布。

```mermaid
flowchart TD
Dev["开发者修改源码"] --> Build["bundle exec jekyll serve --livereload"]
Build --> Preview["浏览器预览 http://127.0.0.1:4000"]
...
- 运行时依赖
  ...
  - Ruby 标准库兼容包（csv、logger、webrick 等），提升跨平台稳定性。
- 前端依赖
...
## 结论
本项目采用清晰的 Jekyll 工程结构与模块化样式/脚本组织方式，具备良好的可维护性与扩展性。遵循本文的开发规范与工作流，新贡献者可快速上手并高效协作。建议在后续迭代中持续完善文档、统一命名与错误提示，并逐步补充自动化测试与质量门禁。
...
- 本地启动
  - 执行 run_server.sh 启动 Jekyll 服务，访问 http://127.0.0.1:4000 预览。
